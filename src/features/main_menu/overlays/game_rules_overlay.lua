...
local GameRulesOverviewText = require("src.features.main_menu.overlays.game_rules_overview_text")
...
local function getOverviewPageText()
    return GameRulesOverviewText.get(I18n:getLocale()) or {}
end
...
local function withFontLineHeight(font, multiplier, draw_fn)
    if not font or not draw_fn then
        return
    end

    if not font.setLineHeight or not font.getLineHeight or not multiplier then
        draw_fn()
        return
    end

    local previous = font:getLineHeight()
    font:setLineHeight(multiplier)
    local ok, err = pcall(draw_fn)
    font:setLineHeight(previous)
    if not ok then
        error(err)
    end
end

local function getLinePixelHeight(font, multiplier)
    local line_height = tonumber(multiplier) or 1
    return math.max(font:getHeight(), math.floor(font:getHeight() * line_height + 0.5))
end
...
local function buildPages()
    ...
    local page_6 = getOverviewPageText()
...
function GameRulesOverlay:_drawBulletList(lines, frame)
    ...
    local line_height_multiplier = 1.24
    ...
    for index, line in ipairs(lines or {}) do
        ...
        local line_count = getWrapLineCount(font, text_line, frame.width)
        withFontLineHeight(font, line_height_multiplier, function()
            love.graphics.printf(text_line, frame.x, y, frame.width, "left")
        end)
        y = y + line_count * getLinePixelHeight(font, line_height_multiplier) + 6
    end
    return y - frame.y
end

function GameRulesOverlay:_measureTextBlockHeight(font, text_value, width, line_gap, line_height_multiplier)
    local line_count = getWrapLineCount(font, text_value, width)
    return line_count * getLinePixelHeight(font, line_height_multiplier) + (line_gap or 0)
end

function GameRulesOverlay:_getTextPageLayout()
    local locale = tostring(I18n:getLocale() or ""):lower()
    local locale_prefix = locale:sub(1, 2)
    local is_cjk = locale_prefix == "zh" or locale_prefix == "ja" or locale_prefix == "ko"

    return {
        inset_left = 10,
        inset_right = 18,
        title_font_token = "TextBig",
        title_line_height = is_cjk and 1.18 or 1.12,
        title_gap = is_cjk and 12 or 10,
        body_line_height = is_cjk and 1.34 or 1.24,
        body_gap = is_cjk and 10 or 8,
        intro_gap = is_cjk and 16 or 14,
        section_gap = is_cjk and 8 or 6,
        hint_line_height = is_cjk and 1.28 or 1.18,
    }
end
...
function GameRulesOverlay:_measureTextPageHeight(page, width)
    ...
    local layout = self:_getTextPageLayout()
    local text_width = math.max(120, width - layout.inset_left - layout.inset_right)
    local body_font = self.fonts:get("TextSmall")
    local title_font = self.fonts:get(layout.title_font_token)
    local caption_font = self.fonts:get("Caption")
    local total = 0

    if page.intro and page.intro ~= "" then
        total = total + self:_measureTextBlockHeight(
            body_font,
            page.intro,
            text_width,
            layout.intro_gap,
    ...
    for _, section in ipairs(page.sections or {}) do
        ...
        total = total + layout.section_gap
    end
...
function GameRulesOverlay:_drawTextPage(content, page, scroll_offset)
    ...
    local frame = self:getTextScrollFrame()
    local layout = self:_getTextPageLayout()
    local text_x = frame.x + layout.inset_left
    local text_width = math.max(120, frame.width - layout.inset_left - layout.inset_right)
    local body_font = self.fonts:get("TextSmall")
    local title_font = self.fonts:get(layout.title_font_token)
    local caption_font = self.fonts:get("Caption")
    local total_height = self:_measureTextPageHeight(page, frame.width)
    ...
    if page.intro and page.intro ~= "" then
        ...
        love.graphics.setColor(colors.text_secondary)
        withFontLineHeight(body_font, layout.body_line_height, function()
            love.graphics.printf(page.intro, text_x, y, text_width, "left")
        end)
        y = y + self:_measureTextBlockHeight(
            body_font,
            page.intro,
            text_width,
            layout.intro_gap,
    ...
    for _, section in ipairs(page.sections or {}) do
        if section.title and section.title ~= "" then
            ...
            withFontLineHeight(title_font, layout.title_line_height, function()
                love.graphics.printf(section.title, text_x, y, text_width, "left")
            end)
            y = y + self:_measureTextBlockHeight(
                title_font,
                section.title,
        ...
        for _, item in ipairs(section.items or {}) do
            ...
            love.graphics.setColor(colors.text_secondary)
            withFontLineHeight(body_font, layout.body_line_height, function()
                love.graphics.printf(line, text_x, y, text_width, "left")
            end)
            y = y + self:_measureTextBlockHeight(
                body_font,
                line,
    ...
    if page.hint and page.hint ~= "" then
        ...
        withFontLineHeight(caption_font, layout.hint_line_height, function()
            love.graphics.printf(page.hint, text_x, y, text_width, "left")
        end)
    end

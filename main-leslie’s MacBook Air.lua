...
local SHOW_LOCAL_TEST_SERVER_OPTION = AppConfig.isServerSelectable("local")
...
local SERVER_PRESETS = (function()
    local presets = {}
    for server_key, preset in pairs(AppConfig.getServerPresets()) do
        presets[server_key] = copyServerPreset(preset)
    end
    return presets
end)()
...
local function buildServerSettingsPatch(server_key)
    if not AppConfig.isServerSelectable(server_key) then
        return nil
    end
...
local function sanitizeSettingsForStartupMode(settings)
    local sanitized_settings, changed = AppConfig.sanitizeNetworkSettings(settings)
    if changed then
        sanitized_settings = app.settings_store:save(sanitized_settings)
    end
...
local function updatePersistedSettings(patch)
    ...
    updated = sanitizeSettingsForStartupMode(updated)
...
local function readCachedPlayerID()
    local cached_player_id = trimValue((app.settings or {}).cached_player_id)
    if cached_player_id == "" then
        return nil
    end
    if not SteamIDProvider.isRealSteamID(cached_player_id) then
        return nil
    end
    return cached_player_id
end

local function persistCachedPlayerID(player_id)
    local normalized_player_id = trimValue(player_id)
    if normalized_player_id == "" then
        app.settings = updatePersistedSettings({
    ...
    app.settings = updatePersistedSettings({
        cached_player_id = normalized_player_id,
        cached_player_id_source = "guest",
    })
    return normalized_player_id
end

local function resolveCurrentIdentity()
    if app.steam_id_provider and type(app.steam_id_provider.getSteamID) == "function" then
        local steam_id = app.steam_id_provider:getSteamID()
        if steam_id and steam_id ~= "" then
            app.steam_id = steam_id
            return app.steam_id, nil
        end
    end

    local cached_player_id = readCachedPlayerID()
    if cached_player_id then
        app.steam_id = cached_player_id
        return app.steam_id, nil
    end

    local result = createSessionService():allocateGuestIdentity()
    if result.ok and result.player_id and result.player_id ~= "" then
        app.steam_id = persistCachedPlayerID(result.player_id)
        return app.steam_id, nil
    end

    app.steam_id = nil
    return nil, (result and result.message) or "Unable to initialize online identity."
end
...
showProfileEditorPage = function()
    ...
    if not steam_id or steam_id == "" then
        steam_id = select(1, resolveCurrentIdentity())
    end
    setScene(NicknameCheckPage.new({
        fonts = app.fonts,
        backgrounds = app.backgrounds,
        steam_id_provider = app.steam_id_provider,
        resolve_identity = resolveCurrentIdentity,
...
local function showNicknameCheckPage()
    local steam_id = select(1, resolveCurrentIdentity())
    setScene(NicknameCheckPage.new({
        fonts = app.fonts,
        backgrounds = app.backgrounds,
        steam_id_provider = app.steam_id_provider,
...
local function resolveSessionAndEnter()
    local steam_id, identity_error = resolveCurrentIdentity()
    if not steam_id or steam_id == "" then
        setScene(NicknameCheckPage.new({
            fonts = app.fonts,
            backgrounds = app.backgrounds,
...
function love.load()
    ...
    app.settings = sanitizeSettingsForStartupMode(app.settings)
    if app.settings.window_mode ~= "fullscreen" then
